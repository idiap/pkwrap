...
# gmm_dir=exp/$gmm
    ...
    data = 'data'
    exp = 'exp'
    gmm_dir= os.path.join(exp, gmm)
    ali_dir= os.path.join(exp, gmm + '_ali_' + train_set + suffix)
    tree_dir= os.path.join(exp, 'chain' + nnet3_affix, 'tree' + suffix + '_' + tree_affix)
    # lang=data/lang_chain
    lat_dir =os.path.join(exp, 'chain' + nnet3_affix, gmm + '_' + train_set + suffix + '_lats')
    dirname = os.path.join(exp, 'chain' + nnet3_affix, 'tdnn_' + affix + suffix)
    train_data_dir = os.path.join(data, train_set + suffix + '_hires')
...
#           we don't use num of jobs b/c it is 1 for now
    ...
    if stage <= 7:
        ...
        out_dir = os.path.join(dirname, "decode_{}{}".format(data_name, decode_suff))
        graph_dir = args.graph_dir
        graph = "{}/HCLG.fst".format(graph_dir)
        num_jobs = pkwrap.utils.split_data(decode_dir)
        subprocess.run(["queue.pl", "-l", "q1d", "-l", "io_big", "-P", "parole", "-V",
