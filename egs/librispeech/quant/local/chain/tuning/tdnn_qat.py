...
#           load model
            ...
            for i in range(1, 861):    # 860 is num_archives in egs
                print("In iter {}".format(i))
                new_model = train_lfmmi_one_iter(
                                model,
                                "ark:" + os.path.join(args.egs, 'cegs.{}.ark'.format(i)), 
                                den_fst_path, 
                                training_opts, 
                                feat_dim, 
                ...
                model = new_model
                if i % 100 ==0 and i>0:
                    torch.save(new_model.state_dict(), args.new_model + '/{}.pt'.format(i))
            torch.save(new_model.state_dict(), args.new_model + '/final.pt')
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    sys.stderr.write("After converting")
                    sys.stderr.write(str(quantized_model.state_dict().keys()))
                    sys.stderr.write(str(quantized_model.tdnn1))
                ...
                for key, feats in pkwrap.script_utils.feat_reader_gen(args.decode_feats):
                    ...
                    post, _ = quantized_model(feats_with_context)
