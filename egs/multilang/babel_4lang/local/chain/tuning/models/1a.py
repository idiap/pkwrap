...
"""
    multilingual acoustic model using multitask output layer.
    this model tries to replicate kaldi TDNN with 7 layers.
    minibatch will have all languages, but minibatch preparation will be taken
    care by pkwrap
"""
...
def train_lfmmi_one_iter(model, egs_file, den_fst_paths, training_opts, feat_dim, 
    minibatch_size="64", use_gpu=True, lr=0.0001, weight_decay=0.25, frame_shift=0, 
    print_interval=10, context_offset=0):
    ...
    for mb_id, merged_egs in enumerate(pkwrap.chain.prepare_minibatch(egs_file, minibatch_size, 0)):
        ...
        frames_per_seq = pkwrap.kaldi.chain.GetFramesPerSequence(merged_egs)
        total_fps = features.shape[1]
        if total_fps == (frames_per_seq*3)+left_context+right_context:
            left_index=frame_shift
        elif total_fps == (frames_per_seq*3)+left_context+right_context+extra_left_context:
            left_index = extra_left_context+frame_shift
        input_shape = features.shape
        features = features[:,left_index:-4+frame_shift,:]
        features = features.cuda()
        output, xent_output = model(features)
        right_index=0
        if output.shape[1] != frames_per_seq:
            sys.stderr.write("Unexpected number of output frames")
            quit(1)
...
class Net(nn.Module):
    def __init__(self, output_dim, feat_dim, num_langs=1):
        super(Net, self).__init__()
        self.input_dim = feat_dim
        self.output_dim = output_dim
        self.bn = nn.BatchNorm1d(feat_dim, affine=False)
        self.tdnn1 = TDNNBatchNorm(feat_dim, 512, context_len=5) # output is N-4
        self.tdnn2 = TDNNBatchNorm(512, 512, context_len=3) # output is N-7
        self.tdnn3 = TDNNBatchNorm(512, 512, context_len=3, subsampling_factor=3) # output is (N-10)/3
...
if __name__ == '__main__':
        ...
        parser.add_argument("--context-offset", default=0, type=int)
...
#           load model
            ...
            sys.stderr.write("Loaded base model from {}\n".format(base_model))
           ...
           -12 119 132 34
            ...
            -12 29 42 4"""             
            test_values = []
            for ln in test_sizes.split('\n'):
                lns = ln.strip().split()
                print(lns)
                test_values.append(list(map(int, lns)))
            frame_shift = 0
            extra_left_context = 40
            left_context = 12
            right_context = 18
            for min_index, max_index, num_frames, frames_per_seq in test_values:
                features = torch.zeros(1, num_frames, 40)
                total_fps = features.shape[1]
                if total_fps == (frames_per_seq*3)+left_context+right_context:
                    left_index=frame_shift
                elif total_fps == (frames_per_seq*3)+left_context+right_context+extra_left_context:
                    left_index = extra_left_context+frame_shift
                output = model(features[:, left_index:-4,:])
                print(features.shape, output[0].shape, output[0].shape[1], frames_per_seq)
