...
class TDNN(nn.Module):
    def __init__(self, feat_dim, output_dim, context_len=1, subsampling_factor=1):
        ...
        assert context_len%2 == 1 # we support only symmetric contexts
        if self.feat_dim == self.output_dim and self.context_len>1:
            self.is_residual_block = True
            self.mid_idx = (context_len-1)//2
        else:
    ...
    def forward(self, input):
        mb, T, D = input.shape
        l = self.context_len
        padded_input = input.contiguous().view(mb, -1).unfold(1, D*self.context_len, D*self.subsampling_factor).contiguous()
        conv_out = F.relu(self.linear(padded_input))
        if self.is_residual_block:
            x = conv_out + input[:, self.mid_idx:-self.mid_idx:self.subsampling_factor,:]
        else:
            x = conv_out
        return x
